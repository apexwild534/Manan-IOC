...
# Run against a list of IOCs — clean or defanged, it handles both
...
Your input file is just one IOC per line — the script detects whether each line is an IP, domain, URL, or hash automatically, and cleans up any defanged indicators before processing.
...
| **v2.1** | Defang / refang — accepts defanged IOCs (`185(.)199(.)109(.)153`, `hxxp://`, `[.]`), preserves original in `ioc_raw` |
